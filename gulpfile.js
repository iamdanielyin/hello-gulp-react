...
gulp.task('server', function () {
    return gulp.src('server/**/*.js')      //引入所有需处理的JS
...
gulp.task('client:backend:styles', function () {
    return gulp.src('public/backend/styles/**/*.css')    //引入所有客户端样式文件
...
gulp.task('client:frontend:styles', function () {
    return gulp.src('public/frontend/styles/**/*.css')    //引入所有客户端样式文件
...
gulp.task('client:backend:scripts', function () {
    return gulp.src('client/backend/**/*.[js,jsx]')      //引入所有需处理的JS和JSX文件
...
gulp.task('client:frontend:scripts', function () {
    return gulp.src('client/frontend/**/*.[js,jsx]')      //引入所有需处理的JS和JSX文件
        .pipe(babel({presets: ['es2015', 'react']}))
...
gulp.task('client:backend:images', function () {
    return gulp.src('public/backend/images/**/*')        //引入所有需处理的图片文件
        ...
        .pipe(notify({message: '管理后台图片处理任务完成'}));
});
...
gulp.task('watch', function () {
    gulp.watch('server/**/*.js', ['server']);//监听服务端
    gulp.watch('public/frontend/styles/**/*.css', ['client:frontend:styles']);//监听客户端样式
    gulp.watch('public/backend/styles/**/*.css', ['client:backend:styles']);//监听管理后台样式
    gulp.watch('client/frontend/**/*.[js,jsx]', ['client:frontend:scripts']);//监听客户端脚本
    gulp.watch('client/backend/**/*.[js,jsx]', ['client:backend:scripts']);//监听管理后台脚本
    gulp.watch('public/frontend/images/**/*', ['client:frontend:images']);//监听客户端图片资源
    gulp.watch('public/backend/images/**/*', ['client:backend:images']);//监听管理后台图片资源
});
